...
#include "doc_artifacts_list.txt"
		...
		if (cpp_modules_map.find(query_module_input) == cpp_modules_map.end())
		{ // the queried function does not exist in doc
			...
			if (correct != "")
				std::cout << "Did you mean: \033[32;1m" << correct << "\033[0m";
			std::cout << "\n       For help: manp -h" << '\n';
			return 1;
		}
		else
		{ // user query exists in doc
