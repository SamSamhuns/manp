...
int main(int argc, const char *argv[])
{
	...
	signal(SIGABRT, sigabrt_handler); // signal handler for abort traps
...
int recursively_find_files_with_ext(std::string &file_extension, std::string &source_path, std::string &target_path)
{
	...
	if (stat(source_path.c_str(), &path_stat) == 0)
	{
		...
		if (S_ISDIR(path_stat.st_mode))
		{
			...
			if (source_path[source_path.length() - 1] != '/')
				source_path += '/';
			if (DEBUG)
				std::cout << source_path << " is a folder\n";
			...
			{								   // Open the source_path directory
				...
				while (dirReadPointer)
				{
						...
						file_path != std::string(".DS_Store"))
					{
						...
						default: // if dirReadPointer is neither file/folder
							if (DEBUG)
								std::cerr << "\033[31;1mERROR:\033[0m " << pathToFile << " is not a regular file or folder\n";
							break;
						}
					}
					dirReadPointer = readdir(dirPtr);
		...
		{
			if (DEBUG)
				std::cout << source_path << " is a file\n";
			file_to_raw_string_text(file_extension, source_path, target_path);
		}
		else
		{
			if (DEBUG)
				std::cerr << "\033[31;1mERROR:\033[0m " << source_path
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m in opening " << source_path << " : "
					  << errno << '\n';
		return -1;
	}
	return 0;
...
int file_to_raw_string_text(std::string &file_extension, std::string &file_path, std::string &target_path)
{
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m " << file_path << " is not a " << file_extension << " file\n";
		return -1;
	}
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m Cannot open " << file_path << "\n";
		return -1;
	}

	/* Get the file_path to use the C++ map key data struct
		the filename will be the unique key for the c++ map */
	...
	{
		if (DEBUG)
			std::cout << file_path << " already in raw literal string format.\n";
		...
		{
			if (DEBUG)
				std::cerr << "\033[31;1mERROR:\033[0m Cannot write to "
						  << write_target_path << "\n";
			return -2;
		}

		if (DEBUG)
			std::cout << file_path << " successfully written to "
								...
								std::vector<std::string> &line_string_vector)
{
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m Cannot open "
					  << write_target_path << "\n";
		return -1;
	}
