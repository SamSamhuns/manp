...
int main(int argc, const char *argv[])
{
	if (argc != 3)
	{
		std::cerr << "\033[32;1mUsage:\033[0m ./raw_string_to_text [TARGET_FILE_EXTENSION] [PATH_TO_DIR_OR_FILE]\n";
		return -1;
	}
...
int recursively_find_files_with_ext(std::string &file_extension, std::string &dirpath)
{
	...
	if (stat(dirpath.c_str(), &path_stat) == 0)
	{
		...
		{
			if (DEBUG)
				std::cerr << "\033[31;1mERROR:\033[0m " << dirpath
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m in opening " << dirpath << " : "
					  << errno << '\n';
		return -1;
	}
	return 0;
...
int raw_string_text_to_file(std::string &file_extension, std::string &filename)
{
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m " << filename << " is not a txt file\n";
		return -1;
	}
	...
	{
		if (DEBUG)
			std::cerr << "\033[31;1mERROR:\033[0m Cannot open " << filename << "\n";
		return -1;
	}
	...
	{
		if (DEBUG)
			std::cerr << filename << " not in raw string literal text format.\n";
		return -2;
	}
	...
	/* avodi adding the first and last strings which are the DELIMITERS */
	...
	out_ptr.close();
	if (DEBUG)
		std::cerr << filename << " successfully converted to normal text format.\n";
	return 0;
}
