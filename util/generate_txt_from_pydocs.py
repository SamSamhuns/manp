    ...
    with open(filename, "r", encoding="utf-8") as readf:
        ...
        for line in readf:
            if repeat == 1:
                ...
                func_name = (
                    func_name.replace("class ", "").replace("\n", "").replace("@", "")
                )
                ...
                with open(fname, fmode, encoding="utf-8") as writef:
                    if header_inc == 1:
            ...
            if not (line[0].isspace()):
                ...
                header_inc = 0 if os.path.isfile(fname) else 1
                with open(
                    fname, fmode, encoding="utf-8"
                ) as writef:  # Append mode for repeat funcs
                    if header_inc == 1:
                        writef.write(f'"{func_name}"\n')
...
def main():
    if len(sys.argv) != 3:
        print(
            "Usage: python generate_funcs_from_modules [SOURCE_FILENAME] [TARGET_DIRECTORY]"
        )
        return
